...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ ok: true, restored: Array.isArray(updated) ? updated.length : 0, restoredId: targetId });
  } catch (error) {
    console.error('Admin restore booking failed:', error);
    return res.status(500).json({ ok: false, message: error.message || 'Could not restore booking.' });
  }
}
